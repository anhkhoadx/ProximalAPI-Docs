...
-------- | --------- | -------- | -----------
**image** | _string_ | max 65K chars | The path to an image file. Usually hosted on imgur.com, although any other server or image hosting service can be used.
**text** | _text_ | max 65K chars | A generic textual message.
**url** | _string_ | max 65K chars | A generic URL path.
**audio** | _string_ | max 65K chars | A URL path to a streaming media resource: audio, video, pages from YouTube, Vimeo, etc.
